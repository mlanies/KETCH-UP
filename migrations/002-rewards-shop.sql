...
-- Reward shop: призы, которые можно купить за XP
CREATE TABLE IF NOT EXISTS reward_shop (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    description TEXT,
    price_xp INTEGER NOT NULL,
    quantity INTEGER DEFAULT 0, -- всего доступно
    quantity_left INTEGER DEFAULT 0, -- сколько осталось
    is_active BOOLEAN DEFAULT 1,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

-- Reward purchases: история покупок призов пользователями
