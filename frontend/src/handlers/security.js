...
export function checkSuspiciousActivity(request, env) {
  ...
  // Исключения для тестовых эндпоинтов
  if (url.pathname === '/test-db' || url.pathname === '/test-sheets' || url.pathname === '/test-cocktails') {
    return { allowed: true, reason: 'Test endpoint - allowed' };
  }
  
  // Исключения для API эндпоинтов системы обучения
  if (url.pathname === '/user-stats' || url.pathname === '/user-achievements' || url.pathname === '/daily-challenges') {
    return { allowed: true, reason: 'Learning API endpoint - allowed' };
  }
  
  // Исключения для miniweb страниц в локальной разработке
  if (url.pathname.startsWith('/miniweb') && (url.hostname === 'localhost' || url.hostname === '127.0.0.1')) {
    return { allowed: true, reason: 'Miniweb local development - allowed' };
  }
  
  // Исключения для API эндпоинтов в локальной разработке
  if ((url.pathname.startsWith('/user-') || url.pathname.startsWith('/daily-') || url.pathname.startsWith('/wine-')) && 
      (url.hostname === 'localhost' || url.hostname === '127.0.0.1')) {
    return { allowed: true, reason: 'API local development - allowed' };
  }
